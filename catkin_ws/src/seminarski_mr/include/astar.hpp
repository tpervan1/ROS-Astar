...
std::vector<Point> aStar(
  ...
  std::map<Point, cv::Point, PointCompare> cameFrom;
...
void drawPath(cv::Mat &image, const std::vector<Point> &path, const Scalar &color) {
  for (long unsigned int i = 1; i < path.size(); i++) {
    cv::line(image, path[i - 1], path[i], color, 1, cv::LINE_AA);
  }
}

std::pair<cv::Mat, cv::Mat> processMapForAStar(std::string filename) {
    Mat map = cv::imread(filename, cv::IMREAD_GRAYSCALE);
    if (map.empty()) {
...
vector<cv::Point> runAll(Point start){

    // Load the map and process it for A*
    std::pair<cv::Mat, cv::Mat> maps = processMapForAStar("/home/tarik/map01.pgm");
    ...
    vector<cv::Point> path = aStar(graph, start, goal);
    ...
    if (!path.empty()) {
        ...
        drawPath(smallerMap, path, cv::Scalar(0, 0, 255));  // Draw path in red
